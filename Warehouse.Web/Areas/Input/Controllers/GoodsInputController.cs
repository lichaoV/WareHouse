...
{
    [Area("Input")]
    [ActionDescription("货品入库")]
    public partial class GoodsInputController : BaseController
    {
        #region 搜索
        [ActionDescription("搜索")]
        public ActionResult Index()
        {
            var vm = CreateVM<GoodsInputListVM>();
            return PartialView(vm);
        }

        [ActionDescription("搜索")]
        [HttpPost]
        public string Search(GoodsInputListVM vm)
        {
        ...
        {
            var vm = CreateVM<GoodsInputVM>(id);
            return PartialView(vm);
        }
        ...
        public ActionResult Edit(GoodsInputVM vm)
        {
            string sql = "SELECT * FROM GoodsInfos WHERE ID='{0}'";
            sql = string.Format(sql, vm.Entity.GoodsInfoId);
            DataTable dt = DC.RunSQL(sql);
            ...
            sql = "SELECT * FROM GoodsInputs WHERE ID='{0}'";
            sql = string.Format(sql, vm.Entity.ID);
            DataTable dt1 = DC.RunSQL(sql);
            decimal oldInputNumber = new decimal();
            decimal newInputNumber = vm.Entity.InputNumber;
            if (dt1.Rows.Count > 0)
            {
                oldInputNumber = Convert.ToDecimal(dt1.Rows[0]["InputNumber"].ToString());
            }
        ...
        {
            var vm = CreateVM<GoodsInputBatchVM>(Ids: IDs);
            return PartialView(vm);
        }

        [HttpPost]
        [ActionDescription("批量修改")]
        public ActionResult DoBatchEdit(GoodsInputBatchVM vm, IFormCollection nouse)
        {
        ...
        [HttpPost]
        [ActionDescription("批量删除")]
        public ActionResult DoBatchDelete(GoodsInputBatchVM vm, IFormCollection nouse)
        {
        ...
        {
            var vm = CreateVM<GoodsInputImportVM>();
            return PartialView(vm);
        }

        [HttpPost]
        [ActionDescription("导入")]
        public ActionResult Import(GoodsInputImportVM vm, IFormCollection nouse)
        {
        ...
        [HttpPost]
        public IActionResult ExportExcel(GoodsInputListVM vm)
        {
            vm.SearcherMode = vm.Ids != null && vm.Ids.Count > 0 ? ListVMSearchModeEnum.CheckExport : ListVMSearchModeEnum.Export;
            var data = vm.GenerateExcel();
            return File(data, "application/vnd.ms-excel", $"Export_GoodsInput_{DateTime.Now.ToString("yyyy-MM-dd")}.xls");
        }
